...
export default class NotionPlugin extends Plugin {
	...
	async onload() {
		...
		const ribbonIconEl = this.addRibbonIcon('Notion', 'Notion', (evt: MouseEvent) => {
			// Called when the user clicks the icon.
			new Notice('This is a notion plugin!');
		});
	...
	async initNotionClient(){
		const notion = new Client({ auth: this.settings.token, })
		this.notion = notion
		const databaseId = 'db3eadeb77604964946ad313dfbf2aa0';
